...
const reviews = [
  ...
  },
  {
    ...
    reply: "Thanks Karim — we've since added a second inspector for the Palmeraie route so first visits happen within 48 hours.",
  },
  {
    name: "Amal R.",
    initials: "AR",
  ...
  return (
    <div className={`flex items-center gap-0.5 text-gold-dark ${className}`}>
      {Array.from({ length: 5 }, (_, i) => (
        <Star key={i} fill={i < filled ? "currentColor" : "none"} size={size} strokeWidth={1.5} />
      ))}
    </div>
  );
}
    ...
    <section className="bg-white px-5 py-14 sm:px-8 sm:py-20 lg:px-12 lg:py-24" id="results">
      <div className="mx-auto max-w-4xl">
        ...
        <h2 className="max-w-xl font-serif font-medium text-4xl leading-[1.12] tracking-tight sm:text-5xl">
          Owners who handed us <span className="italic text-gold-dark">the keys</span>.
        </h2>

        <div className="mt-14 border border-ink/10 bg-sand/25 p-6 sm:p-8">
          <div className="flex flex-col gap-8 sm:flex-row sm:items-center sm:gap-10">
            <div className="shrink-0 pr-4">
              <p className="font-sans font-bold text-6xl text-ink leading-none">4.9</p>
              <Stars className="mt-2.5" rating={4.9} size={18} />
              <p className="mt-2.5 text-[13px] text-ink/55">{totalReviews} reviews</p>
            </div>
              ...
              {breakdown.map(({ score, count }) => (
                <div className="flex items-center gap-4" key={score}>
                  <div className="h-1.5 flex-1 bg-ink/10">
                    <div className="h-full bg-gold-dark" style={{ width: `${(count / maxCount) * 100}%` }} />
                  </div>
                  <span className="w-8 shrink-0 font-semibold text-ink text-sm">{score}</span>
                  <span className="w-16 shrink-0 text-[13px] text-ink/55">{count} reviews</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-4 divide-y divide-ink/10 border-ink/10 border-t">
          {reviews.map(({ name, initials, timeAgo, rating, text, photos, reply }) => (
            <div className="-mx-4 flex flex-col gap-4 px-4 py-8 transition-colors duration-300 hover:bg-sand/15 sm:flex-row sm:gap-6" key={name}>
              ...
              <div className="flex-1">
                ...
                <p className="text-[15px] text-ink/75 leading-relaxed">{text}</p>
                {photos ? (
                ...
                {reply ? (
                  <div className="mt-4 border-gold-dark/40 border-l-2 pl-4">
                    <p className="font-semibold text-[12px] text-gold-dark uppercase tracking-wide">J&amp;M Housing replied</p>
                    <p className="mt-1 text-[13px] text-ink/60 leading-relaxed">{reply}</p>
                  </div>
                ) : null}
              </div>
            </div>
